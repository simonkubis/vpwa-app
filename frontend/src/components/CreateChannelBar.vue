  ...
  <q-card class="full-width no-shadow">
    <q-card-section>
      ...
      <q-form @submit.prevent="createChannel">
        ...
        <q-input
          filled
          v-model="invitedUsersText"
          label="Invite users (@username, comma separated)"
          color="teal-7"
          dense
          class="q-mb-md"
          @blur="parseInvitedUsers"
        />
...
function createChannel() {
  ...
  alert(`Channel: ${channelName.value}\nInvited: ${invitedUsers.value.join(', ')}`)
}
</script>